...
class SearchTechDocsInput(BaseModel):
    ...
    collection: str = Field(
        default="default", description="The document collection to search in"
    )
...
class SearchTechDocsTool(BaseTool):
    name = "search_default"
    description = "This tool enables the querying of a specialized vector store named ‘Default,’ a repository where users archive valuable technical documentation they have encountered. It is particularly beneficial when engaging with technical subjects or when involved in coding activities. Utilize this search tool to scrutinize the vector store for pertinent context when addressing technical inquiries or tasks. If a term from the user input is unfamiliar but appears to be technical in nature, it is imperative to consult ‘Default’ to ascertain whether relevant information or context is available therein. For your awareness, the information provided is sourced from ‘default,’ and we will refer to this source for any related queries."
    args_schema = SearchTechDocsInput
    return_direct = True

    def _run(self, query: str, collection: str = "default", **kwargs) -> str:
