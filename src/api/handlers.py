...
def handle_process_documents(data: DocumentLoaderRequest) -> DocumentLoaderResponse:
    ...
    try:
        processor = DocumentLoader(
            source_dir=data.source_dir, collection=data.collection
        )
        processor.load_documents()
        return DocumentLoaderResponse(
            status="success", message="Documents processed successfully"
        )
...
def handle_document_search(data: DocumentSearchRequest) -> str:
    ...
    #     raise HTTPException(status_code=400, detail=str(e))
    try:
        # Corrected instantiation: replace `user_input` with `query`
        document_search = DocumentSearch(
            collection=data.collection, query=data.user_input
        )
...
def handle_document_similarity(data: DocumentSimilarityRequest) -> str:
